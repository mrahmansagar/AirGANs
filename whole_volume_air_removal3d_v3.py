# -*- coding: utf-8 -*-
"""
Created on Mon Jun 17 16:58:47 2024

Created on Mon Nov 13 16:46:33 2023

@author: Mmr Sagar
PhD Researcher | MPI-NAT Goettingen, Germany

Removing air of the whole volume. 
This script considers the depth overlap so that the air-bubble(s)
at the edge of the chunk can be handled better.

Skip_depth = is the depth of the chunk at the begining and at the end to be 
skipped while saving an air-removed chunk.

Then the next chunk is adjusted so that it has a overlap of skip_depth*2 in 
both sides.  
...
data_dir = 'D:\\SamData\\recons\\MD_1264_A10_Z6.6mm\\'
...
# save dir 
save_dir = os.path.join(data_dir, 'air3d_removed_slices_v3')
...
def modify_list(lst, n, append_first=True, append_last=False):
    if n < 0:
...
skip_depth = 32
overlap = int(skip_depth * 2)

mod_slices = modify_list(slices, skip_depth)

break_flag = False

for step, chunk in enumerate(chunk_list(mod_slices, chunk_size=chunk_depth, overlap=overlap)):
    print('Processing...', step+1, 'of', len(list(chunk_list(mod_slices, chunk_size=chunk_depth, overlap=overlap))))
    
    if len(chunk) < chunk_depth:
        print('less')
        mod_chunk = modify_list(chunk, chunk_depth-len(chunk), append_first=False, append_last=True)
        print(step, 'process:', len(mod_chunk), mod_chunk)
        process_chunk = mod_chunk
        print(step, 'save', len(chunk), chunk[int(skip_depth) :])
        save_chuck = chunk[int(skip_depth) :]
        break_flag = True
        
    else:
        print('else')
        print(step, 'process:', len(chunk), chunk)
        process_chunk = chunk
        print(step, 'save', len(chunk[int(skip_depth):int(-skip_depth)]), chunk[int(skip_depth):int(-skip_depth)])
        save_chuck = chunk[int(skip_depth):int(-skip_depth)]
        
     
    vol_chunk = np.empty(shape=(len(process_chunk), *imarray.shape), dtype=imarray.dtype)
    prev_imarray = None
    for i, fname in enumerate(tqdm(process_chunk, desc='Loading slices....')):
    ...
    # # considering the skip depth
    if break_flag:
        full_air_rem_chunk = full_air_rem_chunk[int(skip_depth) :]
    else:
        full_air_rem_chunk = full_air_rem_chunk[int(skip_depth):int(-skip_depth)]
    
    print('Saving slices....')
    for i, f in enumerate(save_chuck):
